...
pub struct RuleState {
    ...
    range: TextRange,
    /// Whether the rule can safely rewrite the current call.
    fixable: bool,
}
...
/// Returns `true` when `expr` is obviously not a DOM node receiver.
///
/// This mirrors Unicorn's behavior: the rule reports most receivers and only
/// skips ones that are syntactically impossible to be DOM nodes.
