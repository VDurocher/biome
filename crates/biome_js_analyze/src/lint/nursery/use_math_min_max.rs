...
use biome_js_syntax::{
    AnyJsCallArgument, AnyJsExpression, AnyTsType, JsBinaryOperator, JsCallExpression,
    JsConditionalExpression, JsFormalParameter, JsNewExpression, JsSyntaxToken,
    JsVariableDeclarator, binding_ext::AnyJsBindingDeclaration,
};
...
impl Rule for UseMathMinMax {
    ...
    type State = PreferredMathMethod;
    ...
    fn run(ctx: &RuleContext<Self>) -> Self::Signals {
        ...
        classify_method(&comparison, conditional)
    }
    ...
    fn diagnostic(ctx: &RuleContext<Self>, state: &Self::State) -> Option<RuleDiagnostic> {
        ...
        Some(
            ...
            .note(markup! {
                "Dedicated min/max functions like "<Emphasis>"Math."{state.as_str()}"()"</Emphasis>" are clearer and more concise for this purpose."
            }),
        )
    }
    ...
    fn action(ctx: &RuleContext<Self>, state: &Self::State) -> Option<JsRuleAction> {
        ...
        let callee = AnyJsExpression::JsStaticMemberExpression(make::js_static_member_expression(
            make::js_identifier_expression(make::js_reference_identifier(make::ident("Math")))
                .into(),
            make::token(T![.]),
            make::js_name(make::ident(state.as_str())).into(),
        ));
        ...
        Some(JsRuleAction::new(
            ...
            markup! {
                "Use "<Emphasis>"Math."{state.as_str()}"()"</Emphasis>" instead."
            }
            .to_owned(),
            mutation,
        ))
